...
export default function DiningPage() {
    ...
    return (
        <div className="min-h-screen bg-[#f8f4ff] font-sans transition-colors duration-500 text-sm md:text-base">
            <Navbar />

            <main className="mx-auto max-w-[1440px] px-4 md:px-6 lg:px-12 py-8 md:py-12 space-y-12 md:space-y-20">
                <section>
                    <h2 className="text-xl md:text-4xl font-black md:font-semibold mb-6 md:mb-8 uppercase text-black tracking-tighter">Explore Dining</h2>
                    <div className="flex gap-4 md:gap-6 overflow-x-auto scrollbar-hide pb-4">
                        {diningCategories.map((category, i) => (
                            <div
                                key={i}
                                className="flex-shrink-0 rounded-[30px] border border-[#686868] flex flex-col group cursor-pointer transition-all hover:border-primary overflow-hidden relative"
                                style={{
                ...
                <section>
                    <h2 className="text-xl md:text-3xl font-black tracking-tighter mb-6 md:mb-8 uppercase text-zinc-900">Offers for you</h2>
                    <div className="flex gap-4 md:gap-6 overflow-x-auto scrollbar-hide pb-4">
                        {offers.map((offer, i) => (
                ...
                <section className="space-y-6 md:space-y-8">
                    <h2 className="text-xl md:text-3xl font-black tracking-tighter uppercase text-zinc-900">All Restaurants</h2>

                    <div className="flex items-center gap-2 md:gap-3 overflow-x-auto scrollbar-hide py-2">
                        <button className="flex items-center gap-2 px-4 md:px-6 py-2 md:py-2.5 rounded-full border border-zinc-200 bg-white text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:border-primary shadow-sm active:scale-95 whitespace-nowrap">
                            <SlidersHorizontal size={14} className="text-primary" />
                            <span>Filters</span>
                        </button>
                        {['Top rated', 'Pure Veg', 'Serves Alcohol', '50% OFF'].map((filter) => (
                            <button
                                key={filter}
                                onClick={() => toggleFilter(filter)}
                                className={`px-4 md:px-6 py-2 md:py-2.5 rounded-full border text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all shadow-sm whitespace-nowrap active:scale-95 ${activeFilters.includes(filter)
                                    ? 'bg-primary border-primary text-white shadow-primary/20'
                                    : 'border-zinc-200 bg-white text-zinc-600 hover:border-primary'
                                    }`}
                            >
                                {filter}
                            </button>
